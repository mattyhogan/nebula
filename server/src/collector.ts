...
import type { Flux } from '@luxdb/flux';
...
function getCpuTemp(): number {
    try {
        return Math.round(parseInt(readFileSync('/sys/class/thermal/thermal_zone0/temp', 'utf-8')) / 100) / 10;
    } catch {
        return 0;
    }
}
...
function getDisk() {
    const parts = execSync('df -BM / | tail -1').toString().split(/\s+/);
    return { used: parseInt(parts[2]), total: parseInt(parts[1]) };
}
...
function getNetworkRx(): string {
    try {
        const { rx: bytes } = readNetworkBytes();
...
export async function startCollector(flux: Flux) {
    getCpuUsage();
    const initNet = readNetworkBytes();
    prevRxBytes = initNet.rx;
    prevTxBytes = initNet.tx;
    hasPrevNetwork = false;
    await new Promise((r) => setTimeout(r, 100));

    console.log(`pi collector started, writing metrics every ${INTERVAL / 1000}s`);
    ...
    async function tick() {
        ...
        const payload = {
            service: {
                ...
                icon: 'cpu',
                color: '#3b82f6',
                status: cpu > 85 || temp > 80 ? 'degraded' : 'healthy',
            },
            cards: [
                { id: 'cpu', title: 'CPU Usage', type: 'metric', size: 'sm', order: 0, data: { value: cpu, unit: '%', thresholds: { warn: 60, crit: 85 } } },
                { id: 'temp', title: 'CPU Temp', type: 'metric', size: 'sm', order: 1, data: { value: temp, unit: '\u00b0C', thresholds: { warn: 65, crit: 80 } } },
                { id: 'memory', title: 'Memory', type: 'progress', size: 'md', order: 2, data: { value: mem.used, max: mem.total, label: `${mem.used}/${mem.total} MB` } },
                { id: 'disk', title: 'Disk', type: 'progress', size: 'md', order: 3, data: { value: disk.used, max: disk.total, label: `${disk.used}/${disk.total} MB` } },
                ...
                { id: 'temp-hist', title: 'Temp History', type: 'timeseries', size: 'md', order: 6, data: { points: [...tempHistory], unit: '\u00b0C', max: 90, min: 30 } },
        ...
        try {
            ...
            console.error('collector write error:', (err as Error).message);
        }
    }
